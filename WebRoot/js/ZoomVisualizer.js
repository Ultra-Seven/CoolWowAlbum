	...
	$.fn.ZoomVisualizer = function( method )
	{
		...
		function changeImage (obj_){
			...
			_wrapperResize.fadeOut(300,function(){
				...
				loadImage ();
				var now=$('.ativo').attr("href");
				var temp=now.split("/");//get url of pic
				var picId=temp[1].split(".")//get name of pic
				var use="."+picId[0];
				$('div.comment-list-item').hide();//hide all comments
				$(use).show();//show comments of a pic
			});
		}
