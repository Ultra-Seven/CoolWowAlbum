...
<html>
  ...
  <body style="OVERFLOW-X: hidden; ">
  	...
  	<div class="wrapped">
		...
		<div id="zxMain">		
			<s:iterator var="Pics" value="#session.allPicsZX" status="number">
				<s:if test="%{#number.count%4==1}">
					<ul class="forPic">
				</s:if>
				<li class="containerZX" id=<s:property value="#Pics.shareId"/>>
					<div class="moveZX" id=<s:property value="#Pics.picId"/>>
						<img class="picZX" src=<s:property value="#Pics.picsUrl"/>>
						<div class="showInfor">
							<span class="picname"><s:property value="#Pics.picName"/></span>
						</div>
						<div class="aixin">
							<s:if test="%{#Pics.share==false}">
								<a class="xc-bt-like"></a>
							</s:if>
							<s:else>
								<a class="xc-bt-like xc-liked"></a>
							</s:else>
						</div>
					</div>
				</li>
				<s:if test="%{#number.count%4==0}">
					</ul>
				</s:if>
			</s:iterator>
		</div>		
...
<div id="ratyService3" class="modal hide fade">      
	...
	<script type="text/javascript" src="js/friendShare.js"></script>
